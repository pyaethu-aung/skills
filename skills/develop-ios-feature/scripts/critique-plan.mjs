...
/**
 * critique-plan.mjs
 * Derive the autonomous Phase 5 work-list from the Phase 4 critique's PERSISTED
 * snapshot. Run from the project root.
 *
 * Why: the Phase 4 critique (the skill's own rubric pass over simulator
 * screenshots and the diff) normally hands its findings back to the user, which
 * halts a hands-free (--auto) run. But Phase 4 also writes each critique to
 *   .cache/develop-ios-feature/critique/<timestamp>__<slug>.md
 ...
 * (P2/P3) without waiting on a prompt. The fix per finding comes from the
 * SKILL's Phase 5 severity guidance, applied by the agent to each issue's text.
 *
 * Usage:
 *   node …/critique-plan.mjs                 # newest snapshot in the critique dir
...
const CRITIQUE_DIR = '.cache/develop-ios-feature/critique';
...
if (!existsSync(CRITIQUE_DIR)) {
  console.error(`[critique-plan] No ${CRITIQUE_DIR} directory — run the Phase 4 critique first.`);
  process.exit(2);
}
